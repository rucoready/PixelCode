...
UTask_Dodge02::UTask_Dodge02(FObjectInitializer const& ObjectInitializer)
{
    NodeName = TEXT("Dodge Left");

    static ConstructorHelpers::FObjectFinder<UAnimMontage> montageObj(TEXT("/Script/Engine.AnimMontage'/Game/KMS_AI/Boss_Alpernia/Animations/AS_Dodge02_Montage.AS_Dodge02_Montage'"));
    if (montageObj.Succeeded())
    {
        dodge02 = montageObj.Object;
    }
    bNotifyTick = true;
}

EBTNodeResult::Type UTask_Dodge02::ExecuteTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory)
{
    ...
    if (ABossAIController* bossController = Cast<ABossAIController>(OwnerComp.GetOwner()))
    {
        ...
        if (ControlledPawn)
        {
            ...
            if (dodge02 && boss->GetMesh() && boss->GetMesh()->GetAnimInstance())
            {
                //애니메이션을 실행하되 Delegate로 애니메이션이 끝난후 EBTNodeResult::Succeeded를 리턴
                UAnimInstance* AnimInstance = boss->GetMesh()->GetAnimInstance();

                boss->PlayAnimMontage(dodge02);
...
void UTask_Dodge02::TickTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory, float DeltaSeconds)
{
    // 태스크 시작 시 초기화
    if (currentTime == 0.0f)
    {
